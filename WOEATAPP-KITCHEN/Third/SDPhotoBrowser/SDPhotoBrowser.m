...
-(void)deleteImage{
    ...
    NSLog(@"删除的图片 第%d张",index + 1);
    if (_scrollView.subviews.count > index) {
        SDBrowserImageView *currentImageView = _scrollView.subviews[index];
        if (currentImageView && [currentImageView isKindOfClass:[SDBrowserImageView class]]) {
...
- (void)layoutSubviews
{
    ...
    _deleteButton.frame = CGRectMake(self.bounds.size.width - 30 - 50, self.bounds.size.height - 70, 50, 25);
}
