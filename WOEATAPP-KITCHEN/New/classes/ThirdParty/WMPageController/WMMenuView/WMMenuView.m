...
@property (nonatomic, strong) NSMutableArray *touchButtons;
@end
...
static NSInteger const WMButtonTagBegin = 2223;
...
- (void)slideMenuAtProgress:(CGFloat)progress {
    ...
    if (rate == 0.0) {
        ...
        self.selectIndex = self.selItem.tag - WMMenuItemTagOffset;
...
- (void)resetBadgeFrame:(NSInteger)index {
    ...
    if (badgeView) {
        ...
        CGRect itemFrame = [self.frames[index] CGRectValue];
        float centerX = itemFrame.origin.x + itemFrame.size.width/2;
        badgeFrame.origin.x += centerX - badgeFrame.size.width/2;
...
- (void)addBadgeViews {
    for (int i = 0; i < self.titlesCount; i++) {
        UIView *v = [self viewWithTag:WMBadgeViewTagOffset+i];
...
- (void)calculateItemFrames {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        CGRect frame = CGRectMake(contentWidth, self.itemTopSpace, itemW, self.itemHeight);
        // 记录frame
...
{
    for(UIButton *button in _touchButtons) {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        float nextCenterX = 0;
        if (i) {
            CGRect frameLast =  [self.frames[i-1] CGRectValue];
            preCenterX = frameLast.origin.x + frameLast.size.width/2;
        }
        if (i < self.titlesCount-1) {
            CGRect frameNext =  [self.frames[i+1] CGRectValue];
            nextCenterX = frameNext.origin.x + frameNext.size.width/2;
        }
        ...
        if (i == 0) {
            ...
            buttonFrame.origin.x = (centerX + preCenterX) / 2;
        }
        float right;
        if (i < self.titlesCount-1) {
            right = (centerX + nextCenterX) / 2;
        } else {
            right = self.frame.size.width;
        }
        buttonFrame.size.width = right - buttonFrame.origin.x;
        UIButton *b = [UIButton new];
        b.frame = buttonFrame;
        b.tag = WMButtonTagBegin+i;
        [self addSubview:b];
        [b addTarget:self action:@selector(touchButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
    }
}

- (void)touchButtonTapped:(UIButton *)b
{
    int index = b.tag - WMButtonTagBegin;
    WMMenuItem *item = [self viewWithTag:WMMenuItemTagOffset+index];
...
- (void)wm_addProgressViewWithFrame:(CGRect)frame isTriangle:(BOOL)isTriangle hasBorder:(BOOL)hasBorder hollow:(BOOL)isHollow cornerRadius:(CGFloat)cornerRadius {
    ...
    pView.backgroundColor = UICOLOR(199, 199, 203);
...
- (void)didPressedMenuItem:(WMMenuItem *)menuItem {
    ...
    NSInteger currentIndex = self.selItem.tag - WMMenuItemTagOffset;
    _selectIndex = menuItem.tag - WMMenuItemTagOffset;
    if ([self.delegate respondsToSelector:@selector(menuView:didSelesctedIndex:currentIndex:)]) {
