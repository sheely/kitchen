...
- (void)drawRect:(CGRect)rect {
    ...
    //NSLog(@"self.progress %f, startX %f, rate %f", self.progress, startX, rate);
    ...
    if (self.isTriangle) {
        CGContextMoveToPoint(ctx, startX, 0);
        CGContextAddLineToPoint(ctx, endX, 0);
        CGContextAddLineToPoint(ctx, startX + width / 2.0, height);
...
//    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(startX, lineWidth / 2.0, width, height - lineWidth) cornerRadius:self.cornerRadius];
    UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(startX, lineWidth / 2.0, width, height - lineWidth)];
    ...
    float total = self.frame.size.width;
    for(int i=0; i<self.itemFrames.count-1;i++) {
        CGRect r = CGRectMake((i+1)*total/self.itemFrames.count-1, 0, 1, self.frame.size.height);
        if (i == index) {
            r.origin.x += 1;
        }
...
- (void)setFrame:(CGRect)frame
{
    ...
    //NSLog(@"frame %f,%f", self.frame.origin.x, self.frame.size.width);
}
@end
