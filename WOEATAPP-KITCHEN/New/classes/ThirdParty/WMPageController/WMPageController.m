...
- (void)wm_addMenuView {
    ...
    menuView.progressViewBottomSpace = self.progressViewBottomSpace;
     menuView.itemTopSpace = self.itemTopSpace;
     menuView.itemHeight = self.itemHeight;
...
{
    if (![self.dataSource respondsToSelector:@selector(badgeView:atIndex:selected:)]) {
        return nil;
    }
    BOOL isSelected = NO;
    if (index == menu.selectIndex) {
        isSelected = YES;
    } else {
        isSelected = NO;
    }
    return [self.dataSource badgeView:self atIndex:index selected:isSelected];
}
